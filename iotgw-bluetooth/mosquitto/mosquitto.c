...
int mqttpublishbinary(const char *topic, const uint8_t *bytes, size_t len) {
	int x = msgcount++;
	if (my_publish(g_mosq, &x, topic, len, bytes, 0/*QoS*/, false/*retain*/) == MOSQ_ERR_SUCCESS) {
		return x;
	}
	return 0;
}
...
int mqttpublish(const char *topic, const char *message) {
	int x = msgcount++;
	if (my_publish(g_mosq, &x, topic, strlen(message), message, 0/*QoS*/, false/*retain*/) == MOSQ_ERR_SUCCESS) {
		return x;
	}
	return 0;
}
